...
| **지수 AI 요약** | O | O | `GEMINI_API_KEY` 시크릿 필요 · 아래 운영 시간 적용 |
| **공시 AI 요약** | O | O | 시크릿 필요 · 러너에서는 사이클당 3건으로 감속 |
| **종목 AI 요약** | O | **X** | 호출량이 커서 PC 전용 · 아래 운영 시간 적용 |
| 뉴스 한국어 번역 | Claude CLI | 구글 MT | 품질 차이 있음 (아래 참고) |

**종목 AI 요약을 러너에서 막은 이유**: 호출량이 커서 무료 등급을 빠르게 소진한다.
주말에는 기존 캐시가 표시되고, PC 를 켜면 다시 채워진다.

## AI 요약 운영 시간 (지수·공시·종목 공통)

| 구간 | 동작 |
|---|---|
| 평일 06:00 | 갱신 시작 |
| 평일 06:00~17:00 | **30분마다** 갱신 |
| 평일 17:00 | 마지막 갱신 — 이후 하루치 고정값으로 유지 |
| 평일 17:05~익일 06:00 | **정지** |
| 토·일 | **정지** (금요일 17:00 값이 월요일 06:00 까지 그대로 표시) |

시세·뉴스·공시 수집은 이 창과 무관하게 계속 돌아간다. 멈추는 건 AI 요약뿐이다.

빌드는 5분마다 돌지만 AI 는 30분 간격을 지킨다 (`data/cache/ai_cycle.json` 에 마지막 실행 시각 기록).
하루 최대 23 사이클 · 평일만. 이전(5분마다 전량 호출) 대비 호출량이 10분의 1 수준으로 줄어,
무료 등급에서도 429(할당량 초과)가 날 여지가 크게 줄었다.
...
| 지수 요약이 안 바뀜 | 운영 시간 밖(17:00~06:00·주말)이거나 시크릿 미등록 | 정상 동작일 수 있음 → 평일 06:00 이후 확인 |
